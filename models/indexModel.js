...
exports.calculateRentIndex = (
    ...
    featureGroupFactor =
        ...
        0.2;
    let min = rentIndexValues[1];
    let max = rentIndexValues[2];
    let base = rentIndexValues[0];
    let lowerPrices = false;
    let lowerAmount = 0.0
    // consider iwc, heating, bathroom
    if (!bathRoom && !collectiveHeating && yearCategory < 2) {
        lowerPrices = true;
        lowerAmount = 2.2;
    } else if (!bathRoom || !collectiveHeating) {
        lowerPrices = true;
        if (yearCategory === 0) {
            lowerAmount = 1.41;
        } else if (yearCategory === 1) {
            lowerAmount = 0.43;
        } else if (yearCategory === 2) {
            lowerAmount = 1.45;
        }
    }
    if (lowerPrices){
        console.log('lowerPrices', lowerAmount);
        min -= lowerAmount;
        max -= lowerAmount;
        base -= lowerAmount;
    }

    let rentIndexSpecific = base;
    if (featureGroupFactor > 0) {
        rentIndexSpecific += (max - base) * featureGroupFactor;
    }
    if (featureGroupFactor < 0) {
        rentIndexSpecific -= (base - min) * featureGroupFactor * -1;
    }
